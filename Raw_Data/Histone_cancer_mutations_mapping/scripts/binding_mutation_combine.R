...
library(dplyr)
library(forcats)
png(file="./H4_binding_mutation_sites_combine_nucleosome.png",width=10,height=4,units="in",res=300)
# Create data
binding <-read.table("./H4_binding_sites_combine_consensus.txt",header=FALSE, sep=",", col.names = c("residue","number"))
mutation <-read.table("./H4_mutation_sites_combine_consensus.txt",header=FALSE, sep=",", col.names = c("residue","number"))
binding_hotspots=binding[which(binding["number"] >=5),]
...
# Horizontal version
  ...
  #geom_point(data = binding_hotspots, aes(x=residue, y=number), size=2, color="dark grey", 
  #             fill=alpha("dark orange", 0.3), alpha=0.7, shape=21, stroke=0.5) +  
 #geom_line(size=0.5)+ #coord_flip() + 
