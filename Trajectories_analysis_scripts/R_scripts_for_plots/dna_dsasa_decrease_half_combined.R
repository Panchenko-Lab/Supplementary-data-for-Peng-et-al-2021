...
library(matrixStats)
file = "all"
png(file=paste("./amber/sasa_4500ns/dsasa_",file,"_half_combined.png",sep=""),width=8,height=6,units="in",res=300)
df1<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_1aoi.dat",sep=""),header=TRUE)
df2<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_1eqz.dat",sep=""),header=TRUE)
df3<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_ext2.dat",sep=""),header=TRUE)
df4<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_sym.dat",sep=""),header=TRUE)
df5<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_shuxiang_ext_sym_amber_run1.dat",sep=""),header=TRUE)
df6<-read.table(paste("./amber/sasa_4500ns/dsasa_",file,"_shuxiang_ext_sym_amber_run2.dat",sep=""),header=TRUE)
...
par(mar=c(5,5,2,2))
plot(seq(0,93,1), -mean , type="l" , bty="l" , xlab="Superhelical Location (SHL)" , ylab="Mean SASA changes", xaxt = "none",
      col="black" , lwd=3 , pch=15 , ylim=c(-120,0) ,xaxt = "none", 
